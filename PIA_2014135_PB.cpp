...
void altaOrden()
{
    ...
    if (menu >= 0 && menu < 100 && ordenes[menu].numeroTicket == 0)
    {
        ...
        //precio total
        ordenes[menu].precioTotal = 0;
        for (int i = 0; i < ordenes[menu].totalArticulos; ++i)
        {
            ordenes[menu].precioTotal += ordenes[menu].preciosArticulos[i];
        }
        //propina 
        ordenes[menu].propina = ordenes[menu].precioTotal * 0.15;

        //impuesto 
        ordenes[menu].impuesto = ordenes[menu].precioTotal * 0.10;

        //suma de todo
        ordenes[menu].total = ordenes[menu].precioTotal + ordenes[menu].impuesto + ordenes[menu].propina;
...
void guardarOrdenes(Orden ordenes[], int cantidadOrdenes) //Guardar el txt
{
    ...
    for (int i = 0; i < cantidadOrdenes; ++i)
    {
        if (ordenes[i].numeroTicket != 0)
        {
            ...
            archivo << "Precio Total: " << ordenes[i].precioTotal << endl;
